...
def create_policy(admin=False, disable=False, limit: int = 20, block: list = None):
    """
    创建用户策略
...
class Embyservice(metaclass=Singleton):
    ...
    def __init__(self, url: str, api_key: str, timeout: int = 10, max_retries: int = 1):
        ...
        self.headers = {
            ...
            'Authorization': f'MediaBrowser Token="{self.api_key}", Client="Sakura BOT", Device="Sakura BOT", DeviceId="Sakura BOT", Version="1.0.0"',
    ...
    async def emby_create(self, name: str, days: int) -> Union[Tuple[str, str, datetime], bool]:
        ...
        try:
            ...
            result = await self._request('POST', '/Users/New', json={"Name": name})
    ...
    async def emby_del(self, emby_id: str) -> bool:
        ...
        try:
            ...
            result = await self._request('DELETE', f'/Users/{emby_id}')
            if result.success:
    ...
    async def emby_reset(self, emby_id: str, new_password: str = None) -> bool:
        ...
        try:
            ...
            result = await self._request('POST', f'/Users/{emby_id}/Password', json=pwd_data)
            if not result.success:
            ...
            if new_password is None:
                ...
                result = await self._request('POST', f'/Users/{emby_id}/Password', json=pwd_data2)
                if not result.success:
    ...
    async def terminate_session(self, session_id: str, reason: str = "Unauthorized client detected") -> bool:
        ...
        try:
            ...
            message_result = await self._request('POST', f'/Sessions/{session_id}/Message', json=message_data)
    ...
    async def authority_account(self, tg_id: int, username: str, password: str = None) -> Tuple[bool, Union[str, int]]:
        ...
        try:
            ...
            result = await self._request('POST', '/Users/AuthenticateByName', json=data)
            if result.success and result.data:
    ...
    async def user(self, emby_id: str) -> Tuple[bool, Union[Dict, Dict[str, str]]]:
        ...
        """
        try:
            result = await self._request('GET', f'/Users/{emby_id}')
            if result.success:
    ...
    async def get_emby_user_by_name(self, emby_name: str) -> Tuple[bool, Union[Dict, Dict[str, str]]]:
        ...
        """
        try:
            result = await self._request('GET', f'/Users/Query?NameStartsWithOrGreater={emby_name}')
            if result.success and result.data:
    ...
    async def add_favorite_items(self, emby_id: str, item_id: str) -> bool:
        ...
        """
        try:
            result = await self._request('POST', f'/Users/{emby_id}/FavoriteItems/{item_id}')
            if result.success:
    ...
    async def get_favorite_items(self, emby_id: str, start_index: int = None, limit: int = None) -> Union[Dict, bool]:
        ...
        """
        try:
            url = f"/Users/{emby_id}/Items?Filters=IsFavorite&Recursive=true&IncludeItemTypes=Movie,Series,Episode,Person"
    ...
    async def item_id_name(self, emby_id: str, item_id: str) -> str:
        ...
        """
        try:
            result = await self._request('GET', f'/Users/{emby_id}/Items/{item_id}')
            if result.success and result.data:
    ...
    async def item_id_people(self, item_id: str) -> Tuple[bool, Union[List[Dict], Dict[str, str]]]:
        ...
        """
        try:
            result = await self._request('GET', f'/Items?Ids={item_id}&Fields=People')
            if result.success and result.data:
                items = result.data.get("Items", [])
                if items:
    ...
    async def primary(self, item_id: str, width: int = 200, height: int = 300, quality: int = 90) -> Tuple[bool, Union[bytes, Dict[str, str]]]:
        ...
        """
        try:
            url = f'/Items/{item_id}/Images/Primary?maxHeight={height}&maxWidth={width}&quality={quality}'
            result = await self._request('GET', url)
            if result.success:
    ...
    async def backdrop(self, item_id: str, width: int = 300, quality: int = 90) -> Tuple[bool, Union[bytes, Dict[str, str]]]:
        ...
        """
        try:
            url = f'/Items/{item_id}/Images/Backdrop?maxWidth={width}&quality={quality}'
            result = await self._request('GET', url)
            if result.success:
    ...
    async def get_medias_count() -> str:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                url = f"{emby_url}/Items/Counts?ApiKey={emby_api}"
    ...
    async def get_movies(self, title: str, start: int = 0, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            url = (f"/Items?IncludeItemTypes=Movie,Series"
            ...
            if result.success and result.data:
                ...
                for item in items:
                    ...
                    movie_item = {
                        ...
                        'genres': genres,
                        'photo': f'{self.url}/Items/{item.get("Id")}/Images/Primary?maxHeight=400&maxWidth=600&quality=90',
                        'runtime': runtime,
    ...
    async def get_device_by_deviceid(self, deviceid: str) -> Tuple[bool, Union[Dict, Dict[str, str]]]:
        ...
        """
        try:
            result = await self._request('GET', f'/Devices/Info?Id={deviceid}')
            if result.success:
